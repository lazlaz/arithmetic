...
public class ConsistentHash {
	...
	public void init(String[] servers) {
		...
		// 再添加虚拟节点，遍历LinkedList使用foreach循环效率会比较高[因为：LinkedList是链表，如果用for遍历，每次get(i)的时间复杂度O(n)
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < keys.length; i++) {
			System.out.println("[" + keys[i] + "]的hash值为" + hash.getHash(keys[i]) + ", 被路由到结点[" + hash.getServer(keys[i]) + "]");
		}
	}
}
